...
def preprocess_using_tempfile_and_user_logic(img_bgr: np.ndarray, target_width: int = 1200):
    ...
    try:
        ...
        lap_var = is_blurry(gray)

        if lap_var < BLUR_THRESHOLD:
            proc_gray = deblur_and_sharpen_gray(gray)
            block_size = 25 if lap_var < (BLUR_THRESHOLD * 0.7) else 21
            C = 10
        else:
            clahe2 = cv2.createCLAHE(clipLimit=3.0, tileGridSize=(8, 8))
            cl = clahe2.apply(gray)
            proc_gray = cv2.bilateralFilter(cl, d=9, sigmaColor=75, sigmaSpace=75)
            block_size = 15
            C = 9
        ...
        th = cv2.adaptiveThreshold(proc_gray, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C,
        ...
        with tempfile.NamedTemporaryFile(delete=False, suffix=".png") as t2:
            proc_tmp = t2.name
            cv2.imwrite(proc_tmp, morph)

        final_kernel = cv2.getStructuringElement(cv2.MORPH_RECT, (1, 1))
        final_bin = cv2.morphologyEx(morph, cv2.MORPH_OPEN, final_kernel, iterations=1)

        return final_bin.astype("uint8"), float(angle)
