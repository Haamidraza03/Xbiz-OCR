...
# NEW: DETECTION folder for storing JSON of detected text
DETECTION_DIR = os.path.join("Output", "DETECTION")

os.makedirs(OCR_IMAGE_DIR, exist_ok=True)
os.makedirs(OCR_SORTED_DIR, exist_ok=True)
os.makedirs(OCR_TEXT_DIR, exist_ok=True)
os.makedirs(DETECTION_DIR, exist_ok=True)  # ensure DETECTION directory exists
...
def upload():
    ...
    # NOTE: removed creation of plain text file in Output root as requested
    # out_txt_path = os.path.join("Output", f"{base}_{ts}.txt")
    # out_json_path is now saved under DETECTION_DIR
    out_json_path = os.path.join(DETECTION_DIR, f"{base}_{ts}.json")
    ...
    # write JSON payload into DETECTION_DIR (instead of Output root)
    try:
...
def detect():
    ...
    for f in files:
        ...
        # NOTE: removed creation of plain text file in Output root as requested
        # out_txt_path = os.path.join("Output", f"{base}_{ts}.txt")
        # out_json_path moved to DETECTION_DIR
        out_json_path = os.path.join(DETECTION_DIR, f"{base}_{ts}.json")
        ...
        # write JSON payload into DETECTION_DIR
        try:
            with open(out_json_path, "w", encoding="utf-8") as jf:
                json.dump(json_payload, jf, indent=2, ensure_ascii=False)
        except Exception as e:
            print(f"DEBUG: Failed to write detection json for {name}: {e}")
