...
# New imports for authentication & secure comparison
import getpass
import sys
import hmac
...
# -----------------------------------------------------------------------------
# AUTH: Startup prompt + route-level HTTP Basic Auth
# -----------------------------------------------------------------------------
# How credentials are loaded:
# 1. By default the app attempts to read a config file (JSON) whose path is
#    configurable via env var CONFIG_PATH (default: ./config.json).
#    Expected format:
#    {
#        "username": "admin",
#        "password": "s3cret"
#    }
# 2. If config file not found, fall back to env vars: BASIC_AUTH_USER and BASIC_AUTH_PASS
# 3. The server will prompt at startup for username & password and will only continue
#    to run if the entered credentials match the configured ones.
#
# Additionally every incoming request is protected with HTTP Basic Auth and must
# present the same credentials.

EXPECTED_AUTH_USER: Optional[str] = None
EXPECTED_AUTH_PASS: Optional[str] = None

def load_config():
    """
    Loads expected credentials from config file or environment.
    Returns (user, pass) or (None, None) if not found.
    """
    ...
    if os.path.exists(cfg_path):
        try:
            ...
            logging.warning(f"Failed to read config file {cfg_path}: {e}")
    if not user or not pwd:
        user = os.environ.get("BASIC_AUTH_USER")
        pwd = os.environ.get("BASIC_AUTH_PASS")
    return user, pwd

def check_startup_credentials(expected_user: str, expected_pass: str, max_attempts: int = 3) -> bool:
    """
    Prompt the operator for credentials at startup. Return True if matched.
    """
    attempts = 0
    while attempts < max_attempts:
        try:
            entered_user = input("Enter username to start the app: ").strip()
            entered_pass = getpass.getpass("Enter password: ")
        except (KeyboardInterrupt, EOFError):
            print("\nStartup aborted by user.")
            return False
        if hmac.compare_digest(entered_user or "", expected_user or "") and hmac.compare_digest(entered_pass or "", expected_pass or ""):
            print("Authentication successful. Starting server...")
            return True
        else:
            attempts += 1
            print(f"Invalid credentials ({attempts}/{max_attempts}).")
    print("Maximum attempts exceeded. Exiting.")
    return False

def check_auth_header(username: str, password: str) -> bool:
    if EXPECTED_AUTH_USER is None or EXPECTED_AUTH_PASS is None:
        return False
    return hmac.compare_digest(username, EXPECTED_AUTH_USER) and hmac.compare_digest(password, EXPECTED_AUTH_PASS)
...
def requires_auth(f):
    ...
    def decorated(*args, **kwargs):
        ...
        # Try Authorization header if request.authorization not populated
        if not auth:
            ...
            if auth_header.startswith('Basic '):
                try:
                    ...
                    u, p = creds.split(':', 1)
                    auth = type("AuthObj", (), {"username": u, "password": p})
                except Exception:
                    return _unauthorized_response()
            else:
                return _unauthorized_response()
        if not (auth.username and auth.password and check_auth_header(auth.username, auth.password)):
            return _unauthorized_response()
        return f(*args, **kwargs)
    return decorated

# -----------------------------------------------------------------------------
# (rest of your OCR + helper functions)
# -----------------------------------------------------------------------------
...
if __name__ == "__main__":
    EXPECTED_AUTH_USER, EXPECTED_AUTH_PASS = load_config()
    if EXPECTED_AUTH_USER is None or EXPECTED_AUTH_PASS is None:
        logging.warning("No credentials found in config.json or env vars (BASIC_AUTH_USER / BASIC_AUTH_PASS). The app requires credentials to start.")
        sys.exit(1)

    if not check_startup_credentials(EXPECTED_AUTH_USER, EXPECTED_AUTH_PASS):
        sys.exit(1)
