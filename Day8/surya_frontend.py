# app.py — Surya-aware OCR server that returns detected_text as plain strings only
import os
import re
import time
import json
...
try:
    ...
    cv2 = None
    np = None
    print("OpenCV not available; preprocessing skipped.")
...
def init_surya_once():
    ...
    try:
        # Warn if local file shadows package
...
# Keep robust polygon normalizer if needed (not used for display)
...
def extract_text_items(obj: Any) -> List[Dict[str, Any]]:
    """
    Generic recursive extractor: returns items with 'text' where possible.
    This is used internally; final cleaning step will remove polygon/confidence.
    """
    ...
    if isinstance(obj, (list, tuple)):
        ...
        return found
    # object-like with .dict()/model_dump()
...
def _split_and_clean_text_field(raw_text: str) -> List[str]:
    """
    Replace <br> with newline, unescape html entities, strip tags, split on newline and return non-empty trimmed lines.
    """
    if raw_text is None:
        return []
    # replace <br> with newline (preserve intended line breaks)
    s = _HTML_BR_RE.sub("\n", raw_text)
...
def _extract_surya_text_from_raw(raw: Any) -> List[str]:
    """
    Walk raw (which might be surya predictor object/dict) and find:
      - prediction_details -> text_lines -> [{text, polygon, confidence}, ...]
      - detection_details -> text_lines -> ...
    Collect only the 'text' fields (split <br> -> lines).
    """
    out: List[str] = []
    if raw is None:
        return out
    ...
    def walk(node):
        if isinstance(node, dict):
            # check both prediction_details and detection_details (user sample used prediction_details)
            ...
            if "text_lines" in node and isinstance(node["text_lines"], (list, tuple)):
                for tl in node["text_lines"]:
                    if isinstance(tl, dict):
                        ...
                        for s in splits:
                            out.append(s)
            # some variants use 'lines' or 'lines_text'
            ...
            # recursively search deeper keys
...
# -------------- OCR runner wrappers --------------
def run_surya_and_parse(image_path: str) -> Tuple[List[Dict[str, Any]], str]:
    """
    Run Surya recognition (if available) and return parsed_lines list and full_text.
    parsed_lines elements: {"text": str, "confidence": float|None, "bbox": [[x,y],...]|None}
    This function merges generic extractor results and Surya-specific text_lines.
    """
    init_surya_once()
    ...
    if recognition is None:
        raise RuntimeError("Surya recognition predictor not initialized.")

    pil_img = Image.open(image_path).convert("RGB")
    preds = recognition([pil_img], det_predictor=detection)
    if not preds:
        return [], ""
    ...
    # Surya-specific text_lines extraction (uses prediction_details/detection_details)
    try:
    ...
    # fallback - if still empty and raw contains 'text', use it
    if not parsed_lines:
...
# detection helper functions reused from earlier (kept intact)
...
_COORD_ONLY_RE = re.compile(r'^[\[\]\(\)\d\.,\s\-\:;]+$')  # coordinate-like: numbers, punctuation, brackets
_CONF_NUM_RE = re.compile(r'^(?:conf(?:idence)?[:\s]*)?\d{1,3}(?:\.\d+)?$')  # like "0.98" or "confidence: 0.98"

def _alpha_ratio(s: str) -> float:
    if not s:
        return 0.0
    letters = len(re.findall(r'[A-Za-z\u0900-\u097F]', s))  # include Devanagari range too
...
def _looks_like_coord_or_conf(s: str) -> bool:
    ...
    if not ss:
        return True
    # purely coordinate-like
    if _COORD_ONLY_RE.match(ss):
        return True
    # pure numeric/confidence
...
def clean_parsed_lines_to_strings(parsed_lines: List[Any]) -> List[str]:
    """
    Accepts parsed_lines (list of dicts or strings) and returns deduped list of clean strings:
      - prefers explicit 'text' fields
      - strips HTML tags, replaces <br> with newline
      - removes coordinate/confidence noise
      - includes Devanagari letters and latin letters
    """
    out: List[str] = []
    for it in parsed_lines or []:
            ...
            if isinstance(it.get("text"), str) and it.get("text").strip():
                ...
                for k in ("line", "content", "word", "transcription", "raw_text"):
                    ...
                    if isinstance(v, str) and v.strip():
                        txt = v.strip()
                        break
                # as last resort, check other string properties but ignore 'polygon','bbox','confidence'
                if not txt:
                    for k, v in it.items():
                        kl = k.lower()
        ...
        # Replace <br> and remove other html tags
        txts = _split_and_clean_text_field(txt) if ('<' in txt and '>' in txt) else [txt]

        for s in txts:
            s2 = s.strip()
            # remove outer brackets if they contain little alphabetic content
            if _BRACKET_CHARS.search(s2) and _alpha_ratio(s2) < 0.07:
                # likely coordinates; skip
                continue
            # remove stray brackets but keep content if alphabetic weight is ok
            if _BRACKET_CHARS.search(s2):
                s2 = _BRACKET_CHARS.sub(" ", s2)
                s2 = re.sub(r'\s+', ' ', s2).strip()
            # skip coordinate or confidence-looking strings
...
def process_image_bytes(image_bytes: bytes, fmt: str, original_name: str) -> Dict[str, Any]:
    ...
    try:
        ...
        try:
            ...
            if _surya_available:
                try:
                    parsed, full_text = run_surya_and_parse(preprocessed_path)
        ...
        # 2) Paddle fallback
        if not final_full_text and paddle_ocr is not None:
            try:
        ...
        # As a last fallback, if nothing extracted, try to clean raw full_text:
        ...
        # Document classification uses full_text
        try:
        ...
        response_obj = {
            ...
            # KEY: only plain strings (no polygons/confidence metadata)
            "detected_text": detected_strings,
            "cleaned_text": [],          # optionally filled if you want more processing
        ...
        if files_list and len(files_list) > 0:
            for f in files_list:
                try:
                    ...
                    results.append({"original_name": original_name, "result": response_obj})
                except Exception as e:
                    tb = traceback.format_exc()
                    results.append({"original_name": getattr(f, "filename", None) or "unknown", "error": str(e), "trace": tb})
    ...
    try:
        ...
        for fname in files:
            ...
            try:
                ...
                results.append({"original_name": fname, "result": response_obj})
            except Exception as e:
                tb = traceback.format_exc()
                results.append({"original_name": fname, "error": str(e), "trace": tb})
        return jsonify({"results": results}), 200
    except Exception as e:
        tb = traceback.format_exc()
        return jsonify({"error": str(e), "trace": tb}), 500
