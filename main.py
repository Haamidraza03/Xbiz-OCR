...
# Set up global logging for console
...
try:
    ...
    _HAS_RAPIDFUZZ = True
    logging.info("rapidfuzz available — using token_set_ratio for fuzzy matching.")
except Exception:
    # fallback to difflib
    ...
    logging.info("rapidfuzz not installed — falling back to difflib SequenceMatcher for fuzzy matching.")

ALLOWED = {"png", "jpg", "jpeg", "tiff", "bmp"}

# create OCR subfolders
# NOTE: Removed global os.makedirs as now per txn
...
def extract_words_and_boxes_from_paddle(ocr_raw, logger=logging) -> List[Dict[str, Any]]:
    ...
    logger.debug(f"OCR raw type: {type(ocr_raw)}")
    if isinstance(ocr_raw, list) and ocr_raw:
        logger.debug(f"First element type: {type(ocr_raw[0])}")
        if isinstance(ocr_raw[0], dict):
            logger.debug(f"First element keys: {ocr_raw[0].keys()}")
    ...
    if isinstance(ocr_raw, list) and len(ocr_raw) > 0 and isinstance(ocr_raw[0], dict):
        ...
        logger.debug(f"Found {len(texts)} texts, {len(boxes)} boxes, {len(scores)} scores")
        ...
        for i, text in enumerate(texts):
            if i < len(boxes):
                ...
                if vertices and text:
                    ...
                    logger.debug(f"Added text: '{text}' with {len(vertices)} vertices")
    ...
    logger.debug(f"Extracted {len(out)} words")
    return out
...
def group_words_to_lines(words: List[Dict[str, Any]], tolerance_multiplier=0.6, min_tolerance_px=10, use_baseline=True, logger=logging) -> List[Tuple[float, List[Tuple[float, str]]]]:
    ...
    for w in words:
        ...
        try:
            ...
            logger.debug(f"Error processing word vertices: {e}")
            continue

    if not items:
        logger.debug("No valid items found for line grouping")
        return []
    ...
    try:
        ...
        logger.debug(f"Grouped into {len(final_sorted)} lines")
        return final_sorted
    except Exception as e:
        logger.debug(f"Error in line grouping: {e}")
        return []
...
def create_sorted_text_file_from_words(words: List[Dict[str, Any]], output_path: str, tolerance_multiplier=0.6, min_tolerance_px: int = 10, use_baseline: bool = True, logger=logging) -> str:
    try:
        lines = group_words_to_lines(words, tolerance_multiplier=tolerance_multiplier, min_tolerance_px=min_tolerance_px, use_baseline=use_baseline, logger=logger)
        with open(output_path, "w", encoding="utf-8") as fh:
            for _, elems in lines:
                line_text = " ".join(t for _, t in elems).strip()
                if line_text:  # Only write non-empty lines
                    fh.write(line_text + "\n")
        logger.debug(f"Saved sorted text to {output_path} with {len(lines)} lines")
        return output_path
    except Exception as e:
        logger.debug(f"Error creating sorted text file: {e}")
...
def draw_annotations(img: np.ndarray, texts: List[str], boxes: List[List[List[int]]], out_path: str, confidences: Optional[List[float]] = None, color=(0,200,0), logger=logging) -> str:
    ...
    logger.debug(f"Drawing {len(texts)} texts, {len(boxes)} boxes")
    ...
    for i, text in enumerate(texts):
        ...
        if bbox and len(bbox) >= 3:  # Need at least 3 points for a polygon
            try:
                ...
                any_drawn = True
                logger.debug(f"Drawn box for '{text}'")
                
            except Exception as e:
                logger.debug(f"Error drawing box {i}: {e}")
        ...
        if not any_drawn or (bbox and len(bbox) < 3):
            ...
            left_stack_y += step
            any_drawn = True
            logger.debug(f"Drawn left column for '{text}'")

    if not any_drawn:
        cv2.putText(img_draw, "NO DETECTIONS", (20, max(50, h // 2)), 
                   cv2.FONT_HERSHEY_SIMPLEX, 1.2, (0,0,255), 3, cv2.LINE_AA)
        logger.debug("No detections drawn")
...
def upload():
    ...
    try:
        ...
        logs.info(f"Starting upload processing for file: {name}")
        ...
        try:
            ...
            logs.debug(f"OCR completed, result type: {type(ocr_raw)}")
        except Exception as e:
            try:
                os.remove(tmp_path)
            except Exception:
        ...
        full_text = "\n".join(lines)
        logs.debug(f"Extracted {len(lines)} text lines")

        # --- save plain detected text into OCR_TEXT folder ONLY (removed writing into Output root) ---
        ...
        words = extract_words_and_boxes_from_paddle(ocr_raw, logger=logs)
        logs.debug(f"Extracted {len(words)} words with bounding boxes")

        # create sorted text file under OCR_SORTED
        sorted_txt_path = os.path.join(ocr_sorted_dir, f"{base}_{ts}.txt")
        try:
            create_sorted_text_file_from_words(words, sorted_txt_path, tolerance_multiplier=0.6, min_tolerance_px=10, use_baseline=True, logger=logs)
        except Exception as e:
            logs.debug(f"Error creating sorted file: {e}")
        ...
        try:
            draw_annotations(processed_color, texts_for_draw, boxes_for_draw, annotated_image_path, confidences=confidences, logger=logs)
        except Exception as e:
            logs.debug(f"Error drawing annotations: {e}")
        ...
        # write JSON payload into Assets (detection)
        ...
        try:
            ...
            try:
                ...
                pass
            return jsonify({"error": "failed to write detection json", "detail": str(e)}), 500
        ...
        response_dict = {
            # removed "output_txt" which used to point to Output root text file
            "output_json": detection_json_path,
        ...
        with open(request_json_path, "w", encoding="utf-8") as rjf:
            json.dump(request_data, rjf, indent=2)
        ...
        with open(response_json_path, "w", encoding="utf-8") as respjf:
            json.dump(response_dict, respjf, indent=2)

        logs.info("Upload processing completed")
...
def detect():
    ...
    for f in files:
        ...
        try:
            logs = setup_logger(f"ocr_{base}_{ts}", log_path)
            logs.info("Request ocr is running")
            logs.info(f"Starting detect processing for file: {name}")
            ...
            try:
                ...
                logs.debug(f"Failed to write detection json for {name}: {e}")
                # continue processing (we already have other artifacts)
                pass
            ...
            # Save response json (for this file)
            response_json_path = os.path.join(assets_dir, f"{base}_{ts}_response.json")
            with open(response_json_path, "w", encoding="utf-8") as respjf:
                json.dump(result_dict, respjf, indent=2)

            logs.info("Detect processing completed for file: {name}")
...
def process_single_image(img_bytes, filename):
    ...
    try:
        logs = setup_logger(f"ocr_{base}_{ts}", log_path)
        logs.info("Request ocr is running")
        logs.info(f"Starting process_single_image for file: {name}")
        ...
        # Save request json (simulated as no real request, but for consistency)
        request_json_path = os.path.join(assets_dir, f"{base}_{ts}_request.json")
        request_data = {
            "filename": name
        }
        ...
        logs.info("process_single_image completed for file: {name}")
...
def get_sorted_text():
    ...
    try:
        ...
        # Since now per txn, and OCR_SORTED is under base_dir/OUTPUT/OCR_SORTED, but to keep simple, allow if exists
        # But for security, check if it's under current dir
        allowed_dir = os.path.abspath(os.path.normpath("."))  # root of app
